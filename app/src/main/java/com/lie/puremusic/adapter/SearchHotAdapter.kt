...
class SearchHotAdapter(private val searchHotData: SearchHotData): RecyclerView.Adapter<SearchHotAdapter.ViewHolder>(),
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        if (position in 0..2) {
            holder.tvNumber.setTextColor(ContextCompat.getColor(holder.tvNumber.context, R.color.blue))
        } else {
            holder.tvNumber.setTextColor(ContextCompat.getColor(holder.tvNumber.context, R.color.gery))
        }
